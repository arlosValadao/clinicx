 ...
 */
public class ListarSubespecialidadesController implements Initializable {
    ...
    void editarEspecialidade() throws IOException, CloneNotSupportedException {
        ...
        if (antigaEspecialidade != null) {
            ...
            if (isBotaoConfirmarClicked) {
                Gerenciador.atualizarSubspecialidade(antigaEspecialidade, cloneAntigaEspecialidade);
                carregarTableViewEspecialidades();
                return;
            }
        }
        mostrarMsg("Você precisa selecionar uma subespecialidade", Alert.AlertType.ERROR);
    }
    ...
    void removerEspecialidade(ActionEvent event) {
        ...
        if (especialidade != null) {
            ...
            isEspecialidadeRemovida = Gerenciador.removerSubespecialidade(especialidade.getNome());
            if (isEspecialidadeRemovida) {
                mostrarMsg("Especialidade removida com sucesso", Alert.AlertType.INFORMATION);
                carregarTableViewEspecialidades();
                return;
            }
            mostrarMsg("Ainda há médicos associados à esta subespecialidade", Alert.AlertType.ERROR);
            return;
        }
        mostrarMsg("Você precisa selecionar uma subespecialidade", Alert.AlertType.ERROR);
    }
    ...
    public void carregarTelaMedicosAssociados() throws IOException {
        ...
        especialidade = tableViewEspecialidades.getSelectionModel().getSelectedItem();
        if (especialidade != null) {
            FXMLLoader LMALoader = new FXMLLoader(getClass().getResource("/view/subespecialidade/ListarMedicosAssociados.fxml"));
            Stage stage;
            stage = new Stage();
        ...
        mostrarMsg("Selecione uma subespecialidade", Alert.AlertType.ERROR);
    }
    ...
    public void associarMedico() throws IOException {
        ...
        especialidade = tableViewEspecialidades.getSelectionModel().getSelectedItem();
        if (especialidade != null) {
            FXMLLoader TAMLoader = new FXMLLoader(getClass().getResource("/view/subespecialidade/TelaAssociarMedico.fxml"));
            Stage stage;
            stage = new Stage();
    ...
    private void carregarTableViewEspecialidades() {
        ...
        listEspecialidades = Gerenciador.getSubespecialidades().values().stream().collect(Collectors.toList());
    ...
    private boolean carregarTelaEditarEspecialidade(Especialidade antigaEsp, Especialidade cloneAntigaEsp) throws IOException {
        ...
        TEELoader = new FXMLLoader(getClass().getResource("/view/subespecialidade/TelaEditarSubespecialidade.fxml"));

        Stage stage;
        stage = new Stage();
        stage.setScene(new Scene(TEELoader.load()));

        TelaEditarSubespecialidadeController controllerTEE = TEELoader.getController();
